import { FC } from 'react';
import { TheIconWrapper } from '../icon-wrapper';
import { TheIconProps, TheIconSelfPack } from '../model';

const d: Record<string, string> = {
  d1: 'M6.9999 9.5C6.75926 7.50503 7.91573 7.09687 11.9999 7M11.9999 7C16.1233 7.10058 17.3412 7.59829 16.9999 9.5M11.9999 7V17M9.9999 17H13.9999',
  d2: 'M3.89124 3.89124C2.5 5.28249 2.5 7.52166 2.5 12C2.5 16.4783 2.5 18.7175 3.89124 20.1088C5.28249 21.5 7.52166 21.5 12 21.5C16.4783 21.5 18.7175 21.5 20.1088 20.1088C21.5 18.7175 21.5 16.4783 21.5 12C21.5 7.52166 21.5 5.28249 20.1088 3.89124C18.7175 2.5 16.4783 2.5 12 2.5C7.52166 2.5 5.28249 2.5 3.89124 3.89124Z',
  d3: 'M2.61328 12C2.61328 7.52166 2.61328 5.28249 4.00452 3.89124C5.39577 2.5 7.63494 2.5 12.1133 2.5C16.5916 2.5 18.8308 2.5 20.2221 3.89124C21.6133 5.28249 21.6133 7.52166 21.6133 12C21.6133 16.4783 21.6133 18.7175 20.2221 20.1088C18.8308 21.5 16.5916 21.5 12.1133 21.5C7.63494 21.5 5.39577 21.5 4.00452 20.1088C2.61328 18.7175 2.61328 16.4783 2.61328 12Z',
  d4: 'M7.1122 9.5C6.87156 7.50503 8.02803 7.09687 12.1122 7M12.1122 7C16.2356 7.10058 17.4535 7.59829 17.1122 9.5M12.1122 7V17M10.1122 17H14.1122',
  d5: 'M2.5 12C2.5 7.52166 2.5 5.28249 3.89124 3.89124C5.28249 2.5 7.52166 2.5 12 2.5C16.4783 2.5 18.7175 2.5 20.1088 3.89124C21.5 5.28249 21.5 7.52166 21.5 12C21.5 16.4783 21.5 18.7175 20.1088 20.1088C18.7175 21.5 16.4783 21.5 12 21.5C7.52166 21.5 5.28249 21.5 3.89124 20.1088C2.5 18.7175 2.5 16.4783 2.5 12Z',
  d6: 'M6.9686 9.27494C6.72796 7.27997 8.34871 7.25421 11.9872 7M11.9872 7C15.6922 7.20493 17.3396 7.37323 16.9983 9.27494M11.9872 7V17M10.3447 17H13.6715',
  d7: 'M12.0572 1.75H12.0572C14.2479 1.74999 15.9686 1.74998 17.312 1.93059C18.6886 2.11568 19.7809 2.50272 20.6391 3.36091C21.4973 4.21911 21.8843 5.31137 22.0694 6.68802C22.25 8.03144 22.25 9.75214 22.25 11.9428V12.0572C22.25 14.2479 22.25 15.9686 22.0694 17.312C21.8843 18.6886 21.4973 19.7809 20.6391 20.6391C19.7809 21.4973 18.6886 21.8843 17.312 22.0694C15.9686 22.25 14.2479 22.25 12.0572 22.25H11.9428C9.7521 22.25 8.03144 22.25 6.68802 22.0694C5.31137 21.8843 4.21911 21.4973 3.36091 20.6391C2.50272 19.7809 2.11568 18.6886 1.93059 17.312C1.74998 15.9686 1.74999 14.2479 1.75 12.0572V12.0572V11.9428V11.9428C1.74999 9.75211 1.74998 8.03144 1.93059 6.68802C2.11568 5.31137 2.50272 4.21911 3.36091 3.36091C4.21911 2.50272 5.31137 2.11568 6.68802 1.93059C8.03144 1.74998 9.75212 1.74999 11.9428 1.75H11.9428H12.0572ZM10.6219 7.81117C9.90948 7.87477 9.21378 7.98712 8.53904 8.14428C8.25817 8.20969 8.10468 8.24647 7.99185 8.28604C7.80663 8.33922 7.76317 8.50948 7.76459 8.58796C7.75087 8.72447 7.75 8.90654 7.75 9.22222C7.75 9.63644 7.41422 9.97222 7 9.97222C6.58579 9.97222 6.25 9.63644 6.25 9.22222L6.25 9.19204C6.24997 8.91704 6.24995 8.65853 6.2721 8.43802C6.29686 8.19151 6.35299 7.93992 6.49574 7.69044C6.5853 7.53391 6.71237 7.37385 6.8445 7.25112C7.21145 6.91027 7.66773 6.80546 8.10314 6.70545C9.31826 6.42244 10.6879 6.25 12 6.25C13.3121 6.25 14.6817 6.42244 15.8969 6.70545C16.3323 6.80546 16.7886 6.91027 17.1555 7.25112C17.2876 7.37385 17.4147 7.53391 17.5043 7.69044C17.647 7.93992 17.7031 8.19151 17.7279 8.43802C17.7501 8.65852 17.75 8.91707 17.75 9.19205L17.75 9.22222C17.75 9.63644 17.4142 9.97222 17 9.97222C16.5858 9.97222 16.25 9.63644 16.25 9.22222C16.25 8.90654 16.2491 8.72447 16.2354 8.58796C16.2356 8.50916 16.1903 8.33845 16.0082 8.28604C15.8953 8.24647 15.7418 8.20969 15.461 8.14428C14.7862 7.98712 14.0905 7.87477 13.3781 7.81117C13.0894 7.78539 12.945 7.77251 12.8475 7.86171C12.75 7.95091 12.75 8.09982 12.75 8.39765V15.65C12.75 15.9328 12.75 16.0743 12.8379 16.1621C12.9257 16.25 13.0672 16.25 13.35 16.25H13.6667C14.0809 16.25 14.4167 16.5858 14.4167 17C14.4167 17.4142 14.0809 17.75 13.6667 17.75H10.3333C9.91912 17.75 9.58334 17.4142 9.58334 17C9.58334 16.5858 9.91912 16.25 10.3333 16.25H10.65C10.9328 16.25 11.0743 16.25 11.1621 16.1621C11.25 16.0743 11.25 15.9328 11.25 15.65V8.39765C11.25 8.09982 11.25 7.95091 11.1525 7.86171C11.055 7.77251 10.9106 7.78539 10.6219 7.81117Z',
  d8: 'M12.0572 1.75H12.0572H11.9428H11.9428C9.75212 1.74999 8.03144 1.74998 6.68802 1.93059C5.31137 2.11568 4.21911 2.50272 3.36091 3.36091C2.50272 4.21911 2.11568 5.31137 1.93059 6.68802C1.74998 8.03144 1.74999 9.75211 1.75 11.9428V11.9428V12.0572V12.0572C1.74999 14.2479 1.74998 15.9686 1.93059 17.312C2.11568 18.6886 2.50272 19.7809 3.36091 20.6391C4.21911 21.4973 5.31137 21.8843 6.68802 22.0694C8.03144 22.25 9.7521 22.25 11.9428 22.25H12.0572C14.2479 22.25 15.9686 22.25 17.312 22.0694C18.6886 21.8843 19.7809 21.4973 20.6391 20.6391C21.4973 19.7809 21.8843 18.6886 22.0694 17.312C22.25 15.9686 22.25 14.2479 22.25 12.0572V11.9428C22.25 9.75214 22.25 8.03144 22.0694 6.68802C21.8843 5.31137 21.4973 4.21911 20.6391 3.36091C19.7809 2.50272 18.6886 2.11568 17.312 1.93059C15.9686 1.74998 14.2479 1.74999 12.0572 1.75Z',
  d9: 'M8.53904 8.14428C9.21378 7.98712 9.90948 7.87477 10.6219 7.81117C10.9106 7.78539 11.055 7.77251 11.1525 7.86171C11.25 7.95091 11.25 8.09982 11.25 8.39765V15.65C11.25 15.9328 11.25 16.0743 11.1621 16.1621C11.0743 16.25 10.9328 16.25 10.65 16.25H10.3333C9.91912 16.25 9.58334 16.5858 9.58334 17C9.58334 17.4142 9.91912 17.75 10.3333 17.75H13.6667C14.0809 17.75 14.4167 17.4142 14.4167 17C14.4167 16.5858 14.0809 16.25 13.6667 16.25H13.35C13.0672 16.25 12.9257 16.25 12.8379 16.1621C12.75 16.0743 12.75 15.9328 12.75 15.65V8.39765C12.75 8.09982 12.75 7.95091 12.8475 7.86171C12.945 7.77251 13.0894 7.78539 13.3781 7.81117C14.0905 7.87477 14.7862 7.98712 15.461 8.14428C15.7418 8.20969 15.8953 8.24647 16.0082 8.28604C16.1903 8.33845 16.2356 8.50916 16.2354 8.58796C16.2491 8.72447 16.25 8.90654 16.25 9.22222C16.25 9.63644 16.5858 9.97222 17 9.97222C17.4142 9.97222 17.75 9.63644 17.75 9.22222L17.75 9.19205C17.75 8.91707 17.7501 8.65852 17.7279 8.43802C17.7031 8.19151 17.647 7.93992 17.5043 7.69044C17.4147 7.53391 17.2876 7.37385 17.1555 7.25112C16.7886 6.91027 16.3323 6.80546 15.8969 6.70545C14.6817 6.42244 13.3121 6.25 12 6.25C10.6879 6.25 9.31826 6.42244 8.10314 6.70545C7.66773 6.80546 7.21145 6.91027 6.8445 7.25112C6.71237 7.37385 6.5853 7.53391 6.49574 7.69044C6.35299 7.93992 6.29687 8.19151 6.2721 8.43802C6.24995 8.65853 6.24997 8.91704 6.25 9.19204L6.25 9.22222C6.25 9.63644 6.58579 9.97222 7 9.97222C7.41422 9.97222 7.75 9.63644 7.75 9.22222C7.75 8.90654 7.75087 8.72447 7.76459 8.58796C7.76317 8.50948 7.80663 8.33922 7.99184 8.28604C8.10468 8.24647 8.25818 8.20969 8.53904 8.14428Z',
  d10: 'M21 3H3V21H21V3Z',
  d11: 'M13.9432 16.9385H11.9527M9.95192 16.9385H11.9527M16.9523 9.0529V6.95215H7.03711V9.0529M11.9527 6.95215V16.9385',
  d12: 'M3 2.25C2.80109 2.25 2.61032 2.32902 2.46967 2.46967C2.32902 2.61032 2.25 2.80109 2.25 3V21C2.25 21.4142 2.58579 21.75 3 21.75H21C21.4142 21.75 21.75 21.4142 21.75 21V3C21.75 2.58579 21.4142 2.25 21 2.25H3ZM7 6.25C6.58579 6.25 6.25 6.58579 6.25 7V9H7.75V7.75H11.2505V16.25H10V17.75H14V16.25H12.7505V7.75H16.2505V9H17.7505V7C17.7505 6.58579 17.4147 6.25 17.0005 6.25H7Z',
};

export const IconTextSquareStrokeRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-stroke-rounded IconTextSquareStrokeRounded"
    >
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareDuotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-duotone-rounded IconTextSquareDuotoneRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d3} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d3} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
      />
      <path 
        d={d.d4} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareTwotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-twotone-rounded IconTextSquareTwotoneRounded"
    >
      <path 
        d={d.d5} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
      />
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d6} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareSolidRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-solid-rounded IconTextSquareSolidRounded"
    >
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d7} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareBulkRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-bulk-rounded IconTextSquareBulkRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d8} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d9} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareStrokeSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-stroke-sharp IconTextSquareStrokeSharp"
    >
      <path 
        d={d.d10} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d11} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
      />
    </TheIconWrapper>
  );
};

export const IconTextSquareSolidSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="text-square-solid-sharp IconTextSquareSolidSharp"
    >
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d12} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const iconPackOfTextSquare: TheIconSelfPack = {
  name: 'TextSquare',
  StrokeRounded: IconTextSquareStrokeRounded,
  DuotoneRounded: IconTextSquareDuotoneRounded,
  TwotoneRounded: IconTextSquareTwotoneRounded,
  SolidRounded: IconTextSquareSolidRounded,
  BulkRounded: IconTextSquareBulkRounded,
  StrokeSharp: IconTextSquareStrokeSharp,
  SolidSharp: IconTextSquareSolidSharp,
};