import { TheIconWrapper } from '../icon-wrapper';
import { TheIconProps, TheIconSelfPack } from '../model';

const d = {
  d1: 'M15 8C15.6501 8.06476 16.0876 8.21113 16.4142 8.54192C17 9.13523 17 10.0902 17 12C17 13.9098 17 14.8648 16.4142 15.4581C16.0876 15.7889 15.6501 15.9352 15 16M9 16C8.34994 15.9352 7.91238 15.7889 7.58579 15.4581C7 14.8648 7 13.9098 7 12C7 10.0902 7 9.13523 7.58579 8.54192C7.91238 8.21113 8.34994 8.06476 9 8',
  d2: 'M2.5 12C2.5 7.52166 2.5 5.28249 3.89124 3.89124C5.28249 2.5 7.52166 2.5 12 2.5C16.4783 2.5 18.7175 2.5 20.1088 3.89124C21.5 5.28249 21.5 7.52166 21.5 12C21.5 16.4783 21.5 18.7175 20.1088 20.1088C18.7175 21.5 16.4783 21.5 12 21.5C7.52166 21.5 5.28249 21.5 3.89124 20.1088C2.5 18.7175 2.5 16.4783 2.5 12Z',
  d3: 'M17.312 1.93059C15.9686 1.74998 14.2479 1.74999 12.0572 1.75H11.9428C9.75212 1.74999 8.03144 1.74998 6.68802 1.93059C5.31137 2.11568 4.21911 2.50272 3.36091 3.36091C2.50272 4.21911 2.11568 5.31137 1.93059 6.68802C1.74998 8.03144 1.74999 9.75212 1.75 11.9428V12.0572C1.74999 14.2479 1.74998 15.9686 1.93059 17.312C2.11568 18.6886 2.50272 19.7809 3.36091 20.6391C4.21911 21.4973 5.31137 21.8843 6.68802 22.0694C8.03144 22.25 9.7521 22.25 11.9428 22.25H11.9428H12.0572H12.0572C14.2479 22.25 15.9686 22.25 17.312 22.0694C18.6886 21.8843 19.7809 21.4973 20.6391 20.6391C21.4973 19.7809 21.8843 18.6886 22.0694 17.312C22.25 15.9686 22.25 14.2479 22.25 12.0572V12.0572V11.9428V11.9428C22.25 9.7521 22.25 8.03144 22.0694 6.68802C21.8843 5.31137 21.4973 4.21911 20.6391 3.36091C19.7809 2.50272 18.6886 2.11568 17.312 1.93059ZM9.09914 8.99508C9.64871 8.94032 10.0498 8.45043 9.99508 7.90087C9.94033 7.3513 9.45044 6.95018 8.90087 7.00493C8.16083 7.07866 7.44269 7.26353 6.87419 7.83934C6.3668 8.35324 6.16956 8.98547 6.08192 9.64567C5.99991 10.2635 5.99995 11.0356 6 11.9346L6 12L6 12.0654C5.99995 12.9644 5.99991 13.7365 6.08192 14.3543C6.16956 15.0145 6.3668 15.6468 6.87419 16.1607C7.44269 16.7365 8.16083 16.9213 8.90087 16.9951C9.45044 17.0498 9.94033 16.6487 9.99508 16.0991C10.0498 15.5496 9.64871 15.0597 9.09914 15.0049C8.53907 14.9491 8.38208 14.8413 8.2974 14.7555C8.219 14.6761 8.12334 14.5342 8.06453 14.0912C8.00208 13.6206 8 12.9828 8 12C8 11.0172 8.00208 10.3794 8.06453 9.90885C8.12334 9.4658 8.219 9.32391 8.2974 9.2445C8.38208 9.15873 8.53907 9.05087 9.09914 8.99508ZM15.0991 7.00493C14.5496 6.95018 14.0597 7.3513 14.0049 7.90087C13.9502 8.45043 14.3513 8.94032 14.9009 8.99508C15.4609 9.05087 15.6179 9.15873 15.7026 9.2445C15.781 9.3239 15.8767 9.4658 15.9355 9.90885C15.9979 10.3794 16 11.0172 16 12C16 12.9828 15.9979 13.6206 15.9355 14.0912C15.8767 14.5342 15.781 14.6761 15.7026 14.7555C15.6179 14.8413 15.4609 14.9491 14.9009 15.0049C14.3513 15.0597 13.9502 15.5496 14.0049 16.0991C14.0597 16.6487 14.5496 17.0498 15.0991 16.9951C15.8392 16.9213 16.5573 16.7365 17.1258 16.1607C17.6332 15.6468 17.8304 15.0145 17.9181 14.3543C18.0001 13.7366 18.0001 12.9645 18 12.0655V12.0655V12.0655V12.0654V11.9346V11.9346V11.9345V11.9345C18.0001 11.0355 18.0001 10.2635 17.9181 9.64567C17.8304 8.98547 17.6332 8.35324 17.1258 7.83934C16.5573 7.26353 15.8392 7.07866 15.0991 7.00493Z',
  d4: 'M12.0572 1.75C14.2479 1.74999 15.9686 1.74998 17.312 1.93059C18.6886 2.11568 19.7809 2.50272 20.6391 3.36091C21.4973 4.21911 21.8843 5.31137 22.0694 6.68802C22.25 8.03144 22.25 9.7521 22.25 11.9428V11.9428V12.0572V12.0572C22.25 14.2479 22.25 15.9686 22.0694 17.312C21.8843 18.6886 21.4973 19.7809 20.6391 20.6391C19.7809 21.4973 18.6886 21.8843 17.312 22.0694C15.9686 22.25 14.2479 22.25 12.0572 22.25H12.0572H11.9428H11.9428C9.7521 22.25 8.03144 22.25 6.68802 22.0694C5.31137 21.8843 4.21911 21.4973 3.36091 20.6391C2.50272 19.7809 2.11568 18.6886 1.93059 17.312C1.74998 15.9686 1.74999 14.2479 1.75 12.0572V11.9428C1.74999 9.75212 1.74998 8.03144 1.93059 6.68802C2.11568 5.31137 2.50272 4.21911 3.36091 3.36091C4.21911 2.50272 5.31137 2.11568 6.68802 1.93059C8.03144 1.74998 9.75212 1.74999 11.9428 1.75H12.0572Z',
  d5: 'M9.99508 7.90094C10.0498 8.4505 9.64871 8.9404 9.09914 8.99515C8.53907 9.05095 8.38208 9.1588 8.2974 9.24457C8.219 9.32398 8.12334 9.46587 8.06453 9.90892C8.00208 10.3794 8 11.0172 8 12.0001C8 12.9829 8.00208 13.6207 8.06453 14.0912C8.12334 14.5343 8.219 14.6762 8.2974 14.7556C8.38208 14.8413 8.53907 14.9492 9.09914 15.005C9.64871 15.0598 10.0498 15.5496 9.99508 16.0992C9.94033 16.6488 9.45044 17.0499 8.90087 16.9951C8.16083 16.9214 7.44269 16.7365 6.87418 16.1607C6.3668 15.6468 6.16956 15.0146 6.08192 14.3544C5.99991 13.7366 5.99995 12.9645 6 12.0654L6 12.0001L6 11.9347C5.99995 11.0357 5.99991 10.2636 6.08192 9.64575C6.16956 8.98554 6.3668 8.35332 6.87418 7.83941C7.44269 7.2636 8.16083 7.07873 8.90087 7.005C9.45044 6.95025 9.94033 7.35138 9.99508 7.90094Z',
  d6: 'M14.0049 7.90094C14.0597 7.35137 14.5496 6.95025 15.0991 7.005C15.8392 7.07873 16.5573 7.2636 17.1258 7.83941C17.6332 8.35331 17.8304 8.98554 17.9181 9.64575C18.0001 10.2636 18.0001 11.0357 18 11.9347V12.0654C18.0001 12.9645 18.0001 13.7366 17.9181 14.3544C17.8304 15.0146 17.6332 15.6468 17.1258 16.1607C16.5573 16.7365 15.8392 16.9214 15.0991 16.9951C14.5496 17.0499 14.0597 16.6488 14.0049 16.0992C13.9502 15.5496 14.3513 15.0598 14.9009 15.005C15.4609 14.9492 15.6179 14.8413 15.7026 14.7556C15.781 14.6762 15.8767 14.5343 15.9355 14.0912C15.9979 13.6207 16 12.9829 16 12.0001C16 11.0172 15.9979 10.3794 15.9355 9.90892C15.8767 9.46587 15.781 9.32398 15.7026 9.24457C15.6179 9.1588 15.4609 9.05095 14.9009 8.99515C14.3513 8.9404 13.9502 8.4505 14.0049 7.90094Z',
  d7: 'M14.5 8.00015H17.0001V16L14.5 16.0001M9.5 16H7V8L9.5 8',
  d8: 'M3 21H21V3.00046L3 3V21Z',
  d9: 'M3.00002 2.25C2.8011 2.24999 2.61033 2.32901 2.46968 2.46966C2.32902 2.61032 2.25 2.80108 2.25 3V21C2.25 21.4142 2.58579 21.75 3 21.75H21C21.4142 21.75 21.75 21.4142 21.75 21V3.00046C21.75 2.58626 21.4142 2.25047 21 2.25046L3.00002 2.25ZM7.5 7C7.23478 7 6.98043 7.10536 6.79289 7.29289C6.60536 7.48043 6.5 7.73478 6.5 8V16C6.5 16.5523 6.94772 17 7.5 17H10V15H8.5V9L10 9L10 7L7.5 7ZM14 9.00015H15.5001V15.0001L13.9999 15.0001L14.0001 17.0001L16.5002 17C17.0524 17 17.5001 16.5523 17.5001 16V8.00015C17.5001 7.44786 17.0524 7.00015 16.5001 7.00015H14V9.00015Z',
} as const;

export const Icon2ndBracketSquareStrokeRounded = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-stroke-rounded Icon2ndBracketSquareStrokeRounded"
    >
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareDuotoneRounded = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-duotone-rounded Icon2ndBracketSquareDuotoneRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d2} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareTwotoneRounded = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-twotone-rounded Icon2ndBracketSquareTwotoneRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareSolidRounded = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-solid-rounded Icon2ndBracketSquareSolidRounded"
    >
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d3} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareBulkRounded = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-bulk-rounded Icon2ndBracketSquareBulkRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d4} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d5} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d6} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareStrokeSharp = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-stroke-sharp Icon2ndBracketSquareStrokeSharp"
    >
      <path 
        d={d.d7} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d8} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const Icon2ndBracketSquareSolidSharp = (props: TheIconProps) => {
  return (
    <TheIconWrapper
      {...props}
      name="2nd-bracket-square-solid-sharp Icon2ndBracketSquareSolidSharp"
    >
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d9} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const iconPackOf2ndBracketSquare: TheIconSelfPack = {
  name: '2ndBracketSquare',
  StrokeRounded: Icon2ndBracketSquareStrokeRounded,
  DuotoneRounded: Icon2ndBracketSquareDuotoneRounded,
  TwotoneRounded: Icon2ndBracketSquareTwotoneRounded,
  SolidRounded: Icon2ndBracketSquareSolidRounded,
  BulkRounded: Icon2ndBracketSquareBulkRounded,
  StrokeSharp: Icon2ndBracketSquareStrokeSharp,
  SolidSharp: Icon2ndBracketSquareSolidSharp,
};