import { FC } from 'react';
import { TheIconWrapper } from '../icon-wrapper';
import { TheIconProps, TheIconSelfPack } from '../model';

const d: Record<string, string> = {
  d1: 'M3.5 13V12.1963C3.5 9.22892 3.5 7.74523 3.96894 6.56024C4.72281 4.65521 6.31714 3.15255 8.33836 2.44201C9.59563 2.00003 11.1698 2.00003 14.3182 2.00003C16.1173 2.00003 17.0168 2.00003 17.7352 2.25259C18.8902 2.65861 19.8012 3.51728 20.232 4.60587C20.5 5.283 20.5 6.13082 20.5 7.82646V12.0142V13',
  d2: 'M3.5 12C3.5 10.1591 4.99238 8.66667 6.83333 8.66667C7.49912 8.66667 8.28404 8.78333 8.93137 8.60988C9.50652 8.45576 9.95576 8.00652 10.1099 7.43136C10.2833 6.78404 10.1667 5.99912 10.1667 5.33333C10.1667 3.49238 11.6591 2 13.5 2',
  d3: 'M3.5 22V19M3.5 19V17.2C3.5 16.6343 3.5 16.3515 3.67574 16.1757C3.85147 16 4.13431 16 4.7 16H5.5C6.32843 16 7 16.6716 7 17.5C7 18.3284 6.32843 19 5.5 19H3.5ZM20.5 16H19C18.0572 16 17.5858 16 17.2929 16.2929C17 16.5858 17 17.0572 17 18V19M17 22V19M17 19H19.5M14 19C14 20.6569 12.6569 22 11 22C10.6262 22 10.4392 22 10.3 21.9196C9.96665 21.7272 10 21.3376 10 21V17C10 16.6624 9.96665 16.2728 10.3 16.0804C10.4392 16 10.6262 16 11 16C12.6569 16 14 17.3431 14 19Z',
  d4: 'M9.49879 8.49792C5.89879 8.49792 3.5 9.5 3.5 12.1963V13H20.5V7.82643C20.5 6.13079 20.5 5.28297 20.232 4.60583C19.8012 3.51725 18.8902 2.65858 17.7352 2.25256C17.0168 2 16.1172 2 14.3182 2C13.9881 2 13.6753 2 13.3783 2.00051C9.49879 2.00051 10.5332 5.05948 10.5499 6.55167C10.5909 8.19467 9.8002 8.49792 9.49879 8.49792Z',
  d5: 'M3.72368 13.75C4.26144 13.75 4.69737 13.3195 4.69737 12.7885L4.69738 11.7652C4.69737 11.4982 4.69754 10.8596 4.97816 10.3875C5.15109 10.0965 5.37251 9.85337 5.59996 9.72955C5.96647 9.53003 6.38666 9.41667 6.83333 9.41667C6.98046 9.41667 7.13864 9.42314 7.31624 9.43138L7.38206 9.43446C7.53697 9.44174 7.70556 9.44966 7.8737 9.45315C8.25379 9.46105 8.69733 9.44904 9.12548 9.33432C9.95945 9.11086 10.6109 8.45945 10.8343 7.62548C10.949 7.19733 10.961 6.75379 10.9532 6.3737C10.9497 6.20562 10.9417 6.03692 10.9345 5.88206L10.9314 5.81624C10.9231 5.63864 10.9167 5.48046 10.9167 5.33333C10.9167 4.86564 11.0409 4.42698 11.2583 4.04857C11.385 3.82798 11.6057 3.62609 11.9105 3.44878C12.3726 3.17987 12.926 3.17738 13.2642 3.17585C13.6023 3.17433 13.9591 3.17316 14.2572 3.17316C16.0914 3.17316 16.7582 3.18692 17.2614 3.36158C18.1337 3.66441 18.7997 4.29659 19.11 5.07103C19.1878 5.26511 19.2427 5.52002 19.2721 5.96153C19.3021 6.41159 19.3026 6.98607 19.3026 7.81394V12.7885C19.3026 13.3195 19.7386 13.75 20.2763 13.75C20.8141 13.75 21.25 13.3195 21.25 12.7885V7.78121C21.25 6.99374 21.25 6.35586 21.2153 5.83514C21.1795 5.29867 21.1039 4.82043 20.9208 4.36341C20.3922 3.04443 19.2841 2.02533 17.9072 1.54735C17.0482 1.24915 16.0155 1.24951 14.4607 1.25005L13.5134 1.25012L13.5 1.25L13.4649 1.25015C10.9565 1.25185 9.40174 1.28168 8.1118 1.72949C5.8915 2.50027 4.11806 4.13859 3.27491 6.24261C2.99393 6.94378 2.86942 7.69695 2.80918 8.60017C2.74999 9.48756 2.75 10.5838 2.75 11.9814V12.7885C2.75 13.3195 3.18593 13.75 3.72368 13.75Z',
  d6: 'M10.9769 15.25C10.9851 15.25 10.9932 15.25 11.0014 15.25C13.0724 15.25 14.7514 16.9289 14.7514 19C14.7514 21.0711 13.0724 22.75 11.0014 22.75C10.9932 22.75 10.9851 22.75 10.9769 22.75C10.8114 22.75 10.6411 22.7501 10.4957 22.7369C10.3328 22.7221 10.1292 22.6862 9.92636 22.5691C9.52211 22.3357 9.35412 21.9716 9.29144 21.6639C9.24589 21.4403 9.24908 21.1931 9.25083 21.0567C9.25111 21.0348 9.25136 21.0157 9.25136 21V17C9.25136 16.9843 9.25111 16.9653 9.25083 16.9433C9.24908 16.8069 9.24589 16.5597 9.29144 16.3361C9.35412 16.0284 9.52211 15.6643 9.92636 15.4309C10.1292 15.3138 10.3328 15.2779 10.4957 15.2631C10.6411 15.2499 10.8114 15.25 10.9769 15.25ZM10.7513 16.7514C10.7513 16.7539 10.7512 16.7564 10.7511 16.7589C10.7504 16.7911 10.7503 16.826 10.7506 16.867C10.7506 16.8784 10.7507 16.8913 10.7508 16.9049C10.7511 16.9351 10.7514 16.9691 10.7514 17V21C10.7514 21.0309 10.7511 21.0649 10.7508 21.0951C10.7507 21.1088 10.7506 21.1216 10.7506 21.1331C10.7503 21.1741 10.7504 21.2089 10.7511 21.2411C10.7512 21.2436 10.7513 21.2461 10.7513 21.2486C10.7564 21.2487 10.7615 21.2488 10.7669 21.2489C10.8313 21.25 10.906 21.25 11.0014 21.25C12.244 21.25 13.2514 20.2426 13.2514 19C13.2514 17.7574 12.244 16.75 11.0014 16.75C10.906 16.75 10.8313 16.75 10.7669 16.7511C10.7615 16.7512 10.7564 16.7513 10.7513 16.7514Z',
  d7: 'M18.9553 15.25C18.9701 15.25 18.985 15.25 19 15.25H20.5C20.9142 15.25 21.25 15.5858 21.25 16C21.25 16.4142 20.9142 16.75 20.5 16.75H19C18.5074 16.75 18.2134 16.7516 18.0027 16.7799C17.9061 16.7929 17.8578 16.8082 17.836 16.8172C17.8309 16.8192 17.8276 16.8209 17.8257 16.8219L17.8232 16.8232L17.8219 16.8257C17.8209 16.8276 17.8192 16.8309 17.8172 16.836C17.8082 16.8578 17.7929 16.9061 17.7799 17.0027C17.7516 17.2134 17.75 17.5074 17.75 18V18.25H19.5C19.9142 18.25 20.25 18.5858 20.25 19C20.25 19.4142 19.9142 19.75 19.5 19.75H17.75V22C17.75 22.4142 17.4142 22.75 17 22.75C16.5858 22.75 16.25 22.4142 16.25 22V18C16.25 17.985 16.25 17.9701 16.25 17.9553C16.25 17.5224 16.2499 17.1256 16.2933 16.8028C16.3411 16.4473 16.4535 16.0716 16.7626 15.7626C17.0716 15.4535 17.4473 15.3411 17.8028 15.2933C18.1256 15.2499 18.5224 15.25 18.9553 15.25Z',
  d8: 'M4.66261 15.25C4.6751 15.25 4.68757 15.25 4.70001 15.25H5.50001C6.74266 15.25 7.75001 16.2574 7.75001 17.5C7.75001 18.7427 6.74266 19.75 5.50001 19.75H4.25001V22C4.25001 22.4142 3.91423 22.75 3.50001 22.75C3.0858 22.75 2.75001 22.4142 2.75001 22V17.2C2.75001 17.1876 2.75001 17.1751 2.75001 17.1626C2.74993 16.9126 2.74986 16.656 2.77867 16.4417C2.81182 16.1952 2.89498 15.8959 3.14542 15.6454C3.39587 15.395 3.69516 15.3118 3.94172 15.2787C4.15599 15.2499 4.41263 15.2499 4.66261 15.25ZM4.25001 18.25H5.50001C5.91423 18.25 6.25001 17.9142 6.25001 17.5C6.25001 17.0858 5.91423 16.75 5.50001 16.75H4.70001C4.55581 16.75 4.44236 16.7501 4.34496 16.7525C4.31131 16.7534 4.28178 16.7545 4.25578 16.7558C4.25447 16.7818 4.25337 16.8113 4.25252 16.845C4.25006 16.9424 4.25001 17.0558 4.25001 17.2V18.25Z',
  d9: 'M20.5 2H21.25C21.25 1.58579 20.9142 1.25 20.5 1.25V2ZM10.5 2V1.25H10.1893L9.96967 1.46967L10.5 2ZM3.5 9L2.96967 8.46967L2.75 8.68934V9H3.5ZM10.5 9V9.75H11.25V9H10.5ZM19.75 2V13H21.25V2H19.75ZM20.5 1.25H10.5V2.75H20.5V1.25ZM2.75 9V13H4.25V9H2.75ZM9.96967 1.46967L2.96967 8.46967L4.03033 9.53033L11.0303 2.53033L9.96967 1.46967ZM9.75 2V9H11.25V2H9.75ZM10.5 8.25H3.5V9.75H10.5V8.25Z',
  d10: 'M3.5 15.5V14.75H2.75V15.5H3.5ZM10 21.5H9.25V22.25H10V21.5ZM10 15.5V14.75H9.25V15.5H10ZM17 15.5V14.75C16.5858 14.75 16.25 15.0858 16.25 15.5H17ZM3.5 16.25H5.5V14.75H3.5V16.25ZM4.25 22V18.5H2.75V22H4.25ZM4.25 18.5V15.5H2.75V18.5H4.25ZM5.5 17.75H3.5V19.25H5.5V17.75ZM6.25 17C6.25 17.4142 5.91421 17.75 5.5 17.75V19.25C6.74264 19.25 7.75 18.2426 7.75 17H6.25ZM5.5 16.25C5.91421 16.25 6.25 16.5858 6.25 17H7.75C7.75 15.7574 6.74264 14.75 5.5 14.75V16.25ZM10.75 21.5V15.5H9.25V21.5H10.75ZM17 16.25H20.5V14.75H17V16.25ZM17.75 22V18.5H16.25V22H17.75ZM17.75 18.5V15.5H16.25V18.5H17.75ZM19.5 17.75H17V19.25H19.5V17.75ZM10 22.25H11V20.75H10V22.25ZM10 16.25H11V14.75H10V16.25ZM11 16.25C12.2426 16.25 13.25 17.2574 13.25 18.5H14.75C14.75 16.4289 13.0711 14.75 11 14.75V16.25ZM11 22.25C13.0711 22.25 14.75 20.5711 14.75 18.5H13.25C13.25 19.7426 12.2426 20.75 11 20.75V22.25Z',
  d11: 'M20.5011 12.9921V2.08333C20.5011 2.02814 20.4564 1.9834 20.4013 1.9834H10.5241L3.54102 8.98327V12.9726M10.5241 2.55662V8.97156L4.14241 8.98226',
  d12: 'M3.54008 18.4807L3.5206 15.4766H5.36599C5.93851 15.4766 6.5158 15.7032 6.80935 16.1956C7.1749 16.8088 7.15154 17.2865 6.77731 17.8353C6.48024 18.271 5.94756 18.4629 5.42091 18.4668L3.54008 18.4807ZM3.54008 18.4807L3.52051 21.969M20.5208 15.5H17.1022C17.0467 15.5 17.002 15.5452 17.0024 15.6007L17.0267 18.4905M17.0016 21.9804L17.0267 18.4905M17.0267 18.4905H19.5121M10.028 15.4961L10.0281 21.479H11.0364C12.5701 21.479 13.8555 20.3174 14.013 18.789L14.0242 18.6808C14.055 18.3816 14.0181 18.0794 13.9161 17.7965L13.7725 17.3983C13.5195 16.6965 13.0238 16.0921 12.3363 15.805C11.2989 15.3717 10.8731 15.4628 10.028 15.4961Z',
  d13: 'M20.2763 1.25C20.8141 1.25 21.25 1.68754 21.25 2.22727V13.0682H19.3026V3.20455L11.5132 3.20455L11.5132 10.0455H4.69737L4.69737 13.0682H2.75V8.66338L10.1362 1.25H20.2763Z',
  d14: 'M2.75 15.25H5.5C6.74264 15.25 7.75 16.2574 7.75 17.5C7.75 18.7426 6.74264 19.75 5.5 19.75H4.25V22.75H2.75V15.25ZM4.25 18.25H5.5C5.91421 18.25 6.25 17.9142 6.25 17.5C6.25 17.0858 5.91421 16.75 5.5 16.75H4.25V18.25ZM9.25 15.25H11C13.0711 15.25 14.75 16.9289 14.75 19C14.75 21.0711 13.0711 22.75 11 22.75H9.25V15.25ZM10.75 16.75V21.25H11C12.2426 21.25 13.25 20.2426 13.25 19C13.25 17.7574 12.2426 16.75 11 16.75H10.75ZM16.25 16C16.25 15.5858 16.5858 15.25 17 15.25H21.25V16.75H17.75V18.25H20.25V19.75H17.75V22.75H16.25V16Z',
};

export const IconPdf01StrokeRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-stroke-rounded IconPdf01StrokeRounded"
    >
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d3} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01DuotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-duotone-rounded IconPdf01DuotoneRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d4} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d3} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01TwotoneRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-twotone-rounded IconPdf01TwotoneRounded"
    >
      <path 
        d={d.d1} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        d={d.d2} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d3} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01SolidRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-solid-rounded IconPdf01SolidRounded"
    >
      <path 
        d={d.d5} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d6} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d7} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d8} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01BulkRounded: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-bulk-rounded IconPdf01BulkRounded"
    >
      <path 
        opacity="var(--icon-opacity)" 
        d={d.d5} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d6} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d7} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d8} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01StrokeSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-stroke-sharp IconPdf01StrokeSharp"
    >
      <path 
        d={d.d9} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d10} 
        fill="var(--icon-fill)" 
      />
      <path 
        d={d.d11} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
      />
      <path 
        d={d.d12} 
        stroke="var(--icon-stroke)" 
        strokeWidth="var(--icon-stroke-width)" 
      />
    </TheIconWrapper>
  );
};

export const IconPdf01SolidSharp: FC<TheIconProps> = (props: TheIconProps): JSX.Element => {
  return (
    <TheIconWrapper
      {...props}
      name="pdf-01-solid-sharp IconPdf01SolidSharp"
    >
      <path 
        d={d.d13} 
        fill="var(--icon-fill)" 
      />
      <path 
        fillRule="evenodd" 
        clipRule="evenodd" 
        d={d.d14} 
        fill="var(--icon-fill)" 
      />
    </TheIconWrapper>
  );
};

export const iconPackOfPdf01: TheIconSelfPack = {
  name: 'Pdf01',
  StrokeRounded: IconPdf01StrokeRounded,
  DuotoneRounded: IconPdf01DuotoneRounded,
  TwotoneRounded: IconPdf01TwotoneRounded,
  SolidRounded: IconPdf01SolidRounded,
  BulkRounded: IconPdf01BulkRounded,
  StrokeSharp: IconPdf01StrokeSharp,
  SolidSharp: IconPdf01SolidSharp,
};