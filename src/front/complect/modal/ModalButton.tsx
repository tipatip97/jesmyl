...
export default function ModalButton(topProps: TheModalButtonProps) {
  ...
  const asFunc = (val?: Function | boolean | ReactNode) => (typeof val === 'function' ? val(config) : val);
  ...
  return (
    <button
      ...
      onClick={event => {
        ...
        const clickConfig: ModalConfig = mylib.overlap({ button, event }, config, {
        ...
        onActionClick(button, onClick, clickConfig, config as ModalConfig, event);
      }}
