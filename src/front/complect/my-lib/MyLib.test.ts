...
describe('MyLib', () => {
    ...
    it('stringTemplater:default functions', () => {
        ...
        expect(
            mylib.stringTemplater('Лежит $num $declension{{$num}{яблоко}{яблока}{яблок}}))', { num: 8 })
        ).toBe('Лежит 8 яблок))');
    ...
    it('stringTemplater:custom functions', () => {
        expect(
            mylib.stringTemplater(
                'Сумма $arg и $arg1 равна $sum{{$arg}{$arg1}}',
                { arg: 4, arg1: 3, sum: (first: number, second: number) => first + second }
            )
        ).toBe('Сумма 4 и 3 равна 7');

        expect(
            mylib.stringTemplater(
                'В городе есть $translate{{$shop}{##}} где можно купить $translate{{$gift}}',
                {
                    gift: { txt: 'Подарок' },
                    shop: { txt: 'Магазин' },
        ...
        expect(
            mylib.stringTemplater(
                'Каскад ф-ций $fun{{$fun{{$fun{{$fun{{$fun{{$start}}}} }}}} }}',
                {
                    start: 0,
