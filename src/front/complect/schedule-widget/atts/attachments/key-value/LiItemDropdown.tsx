...
export default function ScheduleKeyValueListAttLiItemDropdown({
    ...
    const filter = (id: number) => !value.includes(id)
        && (isShowAll || !topValues.some((value) => value[0] === id || value[1] === id || (mylib.isArr(value[1]) && value[1].includes(id))));

    const sort = isShowAll
        ? zeroFunc
        : (aid: number, bid: number) => {
            const isAIn = topValues.some((value) => value[0] === aid || value[1] === aid || (mylib.isArr(value[1]) && value[1].includes(aid)));
            const isBIn = topValues.some((value) => value[0] === bid || value[1] === bid || (mylib.isArr(value[1]) && value[1].includes(bid)));
    ...
    const titles = topTitles?.map((title) => {
    ...
    const users = topUsers?.map(user => user.mi + CustomAttUseTaleId.Users).filter(filter).sort(sort).map(map) ?? [];
    const roles = topRoles?.map(user => user.mi + CustomAttUseTaleId.Roles).filter(filter).sort(sort).map(map) ?? [];
    const lists = topLists?.map(user => user.mi + CustomAttUseTaleId.Lists).filter(filter).sort(sort).map(map) ?? [];
