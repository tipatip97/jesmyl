...
export const backSwipableContainerMaker = (goBack: () => void, goForward?: () => void) => {
    ...
    const setTimeOut: (callback: (container: HTMLDivElement, ...args: any[]) => void, ts: number, container: HTMLDivElement, ...args: any[]) => TimeOut = setTimeout;
    ...
    const onTouchEnd: TouchEventHandler<HTMLDivElement> = (event) => {
        ...
        if (isCanGo && isHorizontalMoving) {
            ...
            if (deltaX > minLim || deltaX < -minLim) {
                ...
                setTimeOut(withMaxLeftLimAction, toMaxLeftLimTimeout, container, isForwardDir ? transformOriginForward : transformOriginBack);
