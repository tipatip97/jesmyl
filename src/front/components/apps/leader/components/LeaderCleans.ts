...
import {
  ...
  GameTimerImportable,
  GameTimerImportableWithStrongConfigurable,
  GameTimerMode,
  GameTimerSortDirection,
  RateSortedItem,
} from './games/timers/GameTimer.model';
import {
  LeaderGroupChangable,
  LeaderGroupCreatable,
  LeaderGroupExportable,
  LeaderGroupImportable,
...
export class LeaderCleans {
  ...
  static isTimerRowFinished = (
    timer: GameTimerImportable,
    game: TeamGameImportable | und,
    topRowi: number,
    isSomeOf = true,
  ) => {
    const finishes = timer.finishes || {};

    const teams = this.rateSortedTimerTeams(timer, game).filter(({ rowi }) => topRowi === rowi);
  ...
  static rateSortedTimerTeams = (
    timer?: GameTimerImportable,
    game?: TeamGameImportable,
    isFilterOnlyFinished?: boolean,
    specialSort?: GameTimerSortDirection,
  ): RateSortedItem[] => {
    ...
    if (isFilterOnlyFinished) teamsNet = teamsNet.filter(({ finish }) => finish);
  ...
  static computeMinFromMs(ms: number) {
    ...
    return `${date.getHours() ? `${date.getHours()}::` : ''}${date.getMinutes()}:${('' + date.getSeconds()).padStart(
      2,
      '0',
    )}.${date.getMilliseconds()}`;
  }

  static getTimerConfigurableField = <Key extends keyof GameTimerConfigurable>(
    key: Key,
    timer: GameTimerImportable | nil,
    game: TeamGameImportable | nil,
  ): NonNullable<GameTimerConfigurable[Key]> => {
    return (
      this.getTimerConfigurableFieldAsIs(key, timer, game) ??
      ((key === 'mode'
        ? GameTimerMode.TimerTotal
        : key === 'joins'
          ? 1
          : key === 'sort'
            ? GameTimerSortDirection.Desc
            : []) as never)
    );
  };

  static getTimerConfigurableFieldAsIs = <Key extends keyof GameTimerConfigurable>(
    key: Key,
    timer: GameTimerImportable | nil,
    game: TeamGameImportable | nil,
  ): GameTimerConfigurable[Key] => {
  ...
  static getMembersInGroups = (
    participantListName: 'members' | 'mentors',
    contextParticipants: HumanImportable[],
    wids: number[],
    groups: LeaderGroupImportable[] | und,
  ): { member: HumanImportable; group: LeaderGroupImportable }[] => {
    return (
      groups
        ?.map(
          group =>
  ...
  static getContextFieldValues = (
    context: LeaderContextImportable | und,
    groupFields: Record<string, string> | und,
  ...
  static makeHumansToGroupExecDict = (
    value: number[],
    generals: { contextw: number; groupw: number },
    fieldn: 'mentors' | 'members',
    type: 'add' | 'del',
  ): ClientExecutionDict => {
    return {
  ...
  static replaceMemberToGroup = (
    groupw: number,
    contextw: number,
    humanw: number,
    excludeGroups: LeaderGroupImportable[],
    participantListName: 'members' | 'mentors',
  ) => {
    leaderExer.clear();
    const stack: ClientExecutionDict<unknown>[] = [];
