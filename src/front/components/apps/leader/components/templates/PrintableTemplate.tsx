...
export default function PrintableTemplate({
  ...
  noder?: ReactNode | ((node: (node: ReactNode) => ReactNode, props?: HTMLAttributes<HTMLDivElement>) => ReactNode);
  pager?: ReactNode | ((node: (node: ReactNode) => ReactNode, props?: HTMLAttributes<HTMLDivElement>) => ReactNode);
}) {
  const pageNoder = (page: ReactNode, props?: HTMLAttributes<HTMLDivElement>) => (
    <div
      {...props}
      className={`${
        props?.className || ''
