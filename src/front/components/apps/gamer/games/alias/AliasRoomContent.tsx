...
export default function AliasRoomContent() {
  ...
  const isInactualDicts = useMemo(() => {
    return (
      ...
      AliasHelp.takeLens(null, state.dicts, packs, nounProns).some(
        (len, leni) => state.lens![leni] && state.lens![leni] > len,
      )
    );
  }, [nounProns, packs, state]);

  const inactualDictsMessage = <div className="color--ko">Словари в неактуальном состоянии</div>;
  const [expandContent, isExpand] = useIsExpand(false, inactualDictsMessage);

  if (isInactualDicts && !isIManager) return inactualDictsMessage;
