...
export const AliasRoomTeamsConfigures = ({
    isCantStartRound,
    setIsTeamsRejToStart,
    stateRef,
}: Props) => {
    ...
    const isCantComputeNewTeams = membersInPrevRound?.length !== players?.length
        || (membersInPrevRound
            && players?.some(member => !membersInPrevRound.includes(member.login)));
    ...
    const teamsCount =
        stateRef.current.teamsCount =
        isHystericsMode ? (players?.length || 0) / 2 : teamsCountState;

    const isComputeNewTeams =
        stateRef.current.isComputeNewTeams =
        isCantComputeNewTeams || isComputeNewTeamsState;
    ...
    return <>
        ...
        {isComputeNewTeams && <>
            <div className="flex flex-gap margin-gap-v">
                ...
                />
                {isHystericsMode
                    ? <span className="color--7">{teamsCount} {mylib.declension(teamsCount, 'пара', 'пары', 'пар')}</span>
                    : <KeyboardInput
            ...
            {teamsTitles.map((teamTitle) => <div key={teamTitle} className="margin-giant-gap-l color--3">{teamTitle}</div>)}
        </>}
        {isComputeNewTeamsState && <EvaCheckbox
