...
export default function ChordRedactor() {
    ...
    const chordNodes = useMemo(() => {
        ...
        return MyLib.entries(chordBoxes).map(([chordBase, names]) => {
            return <div key={chordBase}>
                <div className={`sticky chord-base-title ${currentChord[0] === chordBase ? 'text-bold' : ''}`}>{chordBase}</div>
                {names.map((chordName) => {
                    return <div
                        key={chordName}
                        onClick={() => setCurrentChord(chordName)}
                        className={`flex center margin-gap pointer ${currentChord === chordName ? 'text-underline' : ''} ${redactableChords[chordName] ? 'color--3' : ''}`}
                    >{chordName}</div>;
                })}
            </div>
        })
                ...
                <div className="flex column center old-chord">
                    {isNewChord
                        ? <>
                            ...
                            >Вернуться к редактированию</div>
                        </>
                        : currentChord
                            ? <>
