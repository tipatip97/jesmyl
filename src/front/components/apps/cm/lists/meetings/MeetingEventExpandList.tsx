    ...
    const prepareCopyText = () => {
        ...
        events?.forEach((event) => {
            if (copyMode === CopyMode.All ||
                ...
                if (event.contextw === lastContextw)
                    eventGroups.at(-1)?.push(event);
        ...
        return eventGroups.map((group) => {
            return group.map(event => {
                ...
                return `${event.name}${titles.length ? ` (${titles.join(' - ')})` : ''}\n${event.coms?.map(com => `${com.number}. ${com.name.trim()}`).join('\n')}`;
            }).join('\n\n');
        }).join('\n\n');
    };
