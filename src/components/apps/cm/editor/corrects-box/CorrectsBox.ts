...
export class CorrectsBox {
    ...
    remove(errors: Correct[] | null, warnings: Correct[] | null, unknowns: Correct[] | null) {
        ...
        this.warnings = warnings == null ? this.warnings : this.warnings?.filter((correct) => warnings.indexOf(correct) < 0) ?? null;
        this.unknowns = unknowns == null ? this.unknowns : this.unknowns?.filter((correct) => unknowns.indexOf(correct) < 0) ?? null;
    }
