      ...
      content={
        <>
          <button
            ...
            onClick={() => {
              ...
              [cols.cats, cols.coms].flat().forEach(
                (col) =>
                  col &&
                  [col.name, (col as EditableCom).texts || []]
                    .flat()
                    .forEach((text) => {
                      (text || "")
                        .toLowerCase()
              ...
              items={"."
                ...
                .map((_, page): DropdownItem<number> => {
                  ...
                  return {
                    ...
                    color: words.some((word) => eeStorage.get(word) == null)
                      ? "ko"
                      : null,
                  };
                })}
          ...
          {wordList.map((word, wordi) => {
            if (
              currentPage * pageSize > wordi ||
              wordi >= currentPage * pageSize + pageSize
            )
              return null;
                      ...
                      <tr>
                        <th>
                          {radioTitles.map((typeName, type) => (
                            <div key={`ee-word-radio-title_${word}:${type}`}>
                              {typeName}
                            </div>
                          ))}
                        </th>
