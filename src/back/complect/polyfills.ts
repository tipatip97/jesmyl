export const setPolyfills = () => {
    const setArrayProtoMethod = <Name extends keyof typeof Array.prototype>(name: Name, value: typeof Array.prototype[Name]) => {
    ...
    const setStringProtoMethod = <Name extends keyof typeof String.prototype>(name: Name, value: typeof String.prototype[Name]) => {
...
(globalThis as any).inspectComponentProps = (curr: Record<string, any>, print?: boolean) => {
    for (const c in curr) {
        console[curr[c] === prev[c] ? 'warn' : 'error']('>>>>', curr[c] === prev[c] ? 'old' : 'NEW', c, ...(print ? [prev[c], curr[c]] : []));
        prev[c] = curr[c];
    }
};
