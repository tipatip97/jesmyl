...
export class AliasHelp {
    ...
    static takeSpeakerDetails = (state: GamerAliasRoomState, currentSpeakeri?: number) => {
        ...
        if (currentSpeakeri !== undefined) {
            ...
            const currentTeamSpeakersList =
                smylib.mapFilter(currentTeamSpeaks, (speaks, speaksi) => speaks === currentTeamMinSpeak && (currentSpeakeri !== speaksi + startCurrentMembersi) ? speaksi : undefined);
        ...
        const nextTeamSpeakersList = smylib.mapFilter(nextTeamSpeaks, (speaks, speaksi) => speaks === nextTeamMinSpeak ? speaksi : undefined);
            ...
            return smylib.isNum(packs[packi].words)
                ? nounsProns
                    ? Math.min(
                        ...
                        smylib.keys(nounsProns.pronouns).length - 1,
                    )
                    : 0
    ...
    static getTokenizedWordInfos = (
        ...
        if (token === undefined || dicts === undefined || nounsProns === undefined)
            return [];

        if (this.tokenizedWordInfoStorage[token] !== undefined)
            return this.tokenizedWordInfoStorage[token];
        ...
        dicts.forEach((max, packi) => {
            ...
            if (smylib.isNum(packWords)) {
                ...
                const words = smylib.keys(packWords)
                    .slice(0, -1)
                    .filter(word => packWords[word] <= max);


                packInfos.push(this.sortItemsByTokenbit(
                    words
                        .slice(0, lens![packi])
                        .map((word, wordi) => {
                            return {
        ...
        if (packInfos.length < 2) infoLine = packInfos.flat();
            ...
            const diffs = packInfos.map(it => Math.floor((it.length - min) / (max * .1)));
            ...
            while (true) {
                ...
                if (packInfos[infoItemi].length === 0) {
                    ...
                    if (packInfos.length > 1)
                        packPointerTokenbit = computePackPointerTokenbit();
                    else packPointerTokenbit = [0];
                }

                if (stacki > 10000 || packInfos.length === 0)
                    break;
            }
        }
