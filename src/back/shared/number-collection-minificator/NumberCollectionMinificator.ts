...
export function NumberCollectionMinificator<InsideDigits extends [InsideDigit?, InsideDigit?, InsideDigit?]>(
  insideDigits?: InsideDigits,
) {
  ...
  return {
      ...
      if (insideDigits && insideNumbers)
        insideNumbers.forEach((num, numi) => {
          if ((sNum = '' + num).length > insideDigits[numi]!)
            throw new Error(
              `Ошибка ширины цифры! Ожидается максимум ${insideDigits[numi]!} ${smylib.declension(
                insideDigits[numi]!,
                'порядок',
                'порядка',
                'порядков',
              )}, а получено число ${num}`,
            );
    ...
    decode(num: number | nil): [number, InsideNumbers, number] | null {
      ...
      return [
        ...
        (insideDigits
          ? strNumbers.map((num, numi) => +(num?.padEnd(insideDigits[numi]!, '0') || 0)).slice(0, -1)
          : []) as never,
